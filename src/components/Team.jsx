    ...
    <div className={`relative group perspective h-full ${className}`}>
        <div className="w-full h-full min-h-[360px] bg-slate-900/80 border-t-4 border-slate-700 p-6 rounded-b-md overflow-hidden flex flex-col justify-start transition-all duration-500 group-hover:-translate-y-2 group-hover:border-[#305CDE]/80 group-hover:shadow-[0_0_25px_rgba(48,92,222,0.15)] group-hover:bg-slate-900 border-x border-b">
            ...
            <div className="flex flex-col items-center mb-6">
                ...
                <h3 className="text-xl font-bold font-mono text-[#305CDE] tracking-[0.1em] uppercase text-center">{member.codename}</h3>
                <p className="text-sm text-slate-400 font-mono mt-2 tracking-wide block text-center min-h-[2.5rem]">{member.role}</p>
            </div>

            <div className="mt-auto flex justify-center gap-4">
                {member.linkedin && (
                    <a href={member.linkedin} target="_blank" rel="noreferrer" className="text-slate-500 p-2 bg-black/40 border border-[#305CDE]/30 hover:border-[#305CDE] hover:text-[#305CDE] transition-colors group/icon">
                        <Linkedin size={18} className="group-hover/icon:scale-110 transition-transform" />
                    </a>
                )}
                {member.twitter && (
                    <a href={member.twitter} target="_blank" rel="noreferrer" className="text-slate-500 p-2 bg-black/40 border border-[#305CDE]/30 hover:border-[#305CDE] hover:text-[#305CDE] transition-colors group/icon">
                        <FaXTwitter size={18} className="group-hover/icon:scale-110 transition-transform" />
                    </a>
                )}
                {member.instagram && (
                    <a href={member.instagram} target="_blank" rel="noreferrer" className="text-slate-500 p-2 bg-black/40 border border-[#305CDE]/30 hover:border-[#305CDE] hover:text-[#305CDE] transition-colors group/icon">
                        <Instagram size={18} className="group-hover/icon:scale-110 transition-transform" />
                    </a>
                )}
                {member.tiktok && (
                    <a href={member.tiktok} target="_blank" rel="noreferrer" className="text-slate-500 p-2 bg-black/40 border border-[#305CDE]/30 hover:border-[#305CDE] hover:text-[#305CDE] transition-colors group/icon">
                        <FaTiktok size={18} className="group-hover/icon:scale-110 transition-transform" />
                    </a>
                )}
                {member.github && (
                    <a href={member.github} target="_blank" rel="noreferrer" className="text-slate-500 p-2 bg-black/40 border border-[#305CDE]/30 hover:border-[#305CDE] hover:text-[#305CDE] transition-colors group/icon">
                        <FaGithub size={18} className="group-hover/icon:scale-110 transition-transform" />
                    </a>
                )}
                {member.itchio && (
                    <a href={member.itchio} target="_blank" rel="noreferrer" className="text-slate-500 p-2 bg-black/40 border border-[#305CDE]/30 hover:border-[#305CDE] hover:text-[#305CDE] transition-colors group/icon">
            ...
            {/* Scanline effect */}
            <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#305CDE]/5 to-transparent opacity-0 group-hover:opacity-100 group-hover:animate-scanline pointer-events-none" />
        </div>
    </div>
);
    ...
    return (
        <section id="team" className="w-full py-24 bg-slate-950 border-b border-slate-800 text-white relative">
            <div className="container mx-auto px-6 relative z-10">
                <h2 className="text-3xl font-mono font-bold tracking-widest text-[#305CDE] mb-16 text-center">
                    Our Team
                </h2>
